...
const fileItem = tv({
  slots: {
    ...
    icon: 'relative rounded-full border-4 border-sky-100 bg-sky-200 p-2 text-sky-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500',
  ...
  variants: {
    state: {
      ...
      complete: {
        base: 'border-sky-500 dark:border-sky-300/20',
      },
      error: {
...
export function FileItem({ state, name, size, type }: FileItemProps) {
  ...
  return (
    <div className={base()}>
        ...
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <div className="flex w-full items-center gap-3">
            <div className="h-2 flex-1 rounded-full bg-zinc-100 dark:bg-zinc-600">
              <div
                className="h-2 rounded-full bg-sky-600 dark:bg-sky-400"
      ...
      {state === 'complete' ? (
        <CheckCircle2 className="h-5 w-5 fill-sky-600 text-white dark:fill-sky-300 dark:text-zinc-900" />
      ) : (
